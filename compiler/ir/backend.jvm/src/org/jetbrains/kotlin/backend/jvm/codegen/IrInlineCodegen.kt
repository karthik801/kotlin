    ...
    override fun genValueAndPut(
        ...
        if (irValueParameter?.isInlineParameter() == true && isInlineIrExpression(argumentExpression)) {
            ...
            if (boundReceiver != null) {
                activeLambda = lambdaInfo
                putCapturedValueOnStack(boundReceiver.initializer!!, lambdaInfo.capturedParamsInDesc[0], 0)
                activeLambda = null
            }
        } else {
...
fun isInlineIrExpression(argumentExpression: IrExpression) =
    when (argumentExpression) {
        ...
        is IrCallableReference -> true.also {
            assert(argumentExpression.getArgumentsWithIr().count() <= 1) {
                "Expecting no more than 1 bounded argument for callable reference: ${argumentExpression.dump()}"
            }
        }
        else -> false
    }
